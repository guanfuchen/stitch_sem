...
def main1():
	images = glob.glob(sys.argv[1]+'*.bmp')
	if len(sys.argv) == 3:
		labels = glob.glob(sys.argv[2]+'*.bmp')
	elif len(sys.argv) == 2:
		labels = glob.glob(sys.argv[1]+'*.bmp')
	images.sort(reverse=True)
	labels.sort(reverse=True)

	# print(images)
	# print(labels)
	curr_image = cv2.imread(images[0])
	curr_label = cv2.imread(labels[0])
	...
	for image_id, image in enumerate(images[1:]):
		...
		if M is not None:
			...
			curr_label = get_stitched_image(curr_label, prev_label, M)
	print('show result')
	cv2.imshow('curr_image', curr_image)
	cv2.imshow('curr_label', curr_label)
	cv2.waitKey()
